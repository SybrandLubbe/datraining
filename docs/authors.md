...
This guide is for **content authors** — no coding experience needed. It explains what da.live is, how pages are organized, how to find and use content blocks, how to build a page for a specific country/language, and how to preview and publish your work.
...
You never touch code. You author a document, and the block's existing code automatically styles whatever you put in the table. For a plain-English explanation of **exactly what each block does and how it renders your content**, see the [Blocks Library](../pages/blocks.html) — every block there has a "What it does / How it works / When to use it" writeup.
...
For every block you'll find a plain-English explanation of **what it does, how it works under the hood, and when to reach for it** — not just a snippet. Use the search box or category filter to jump straight to the one you need, and the **"Try it"** panel to fill in sample fields and see a rendered preview before you commit to it in da.live.

The blocks currently available in the Bimota project (from the [public blocks repo](https://github.com/kawaind/bimota/tree/main/blocks)) are grouped into four categories in the library:

| Category | Blocks |
|---|---|
| **Layout & content** | Hero, Title, Text, Text with Image, Columns, Cards, Teaser, News/Stories, Feature, Highlight, Gallery, Carousel, Image |
| **Product & specs** | Vehicle Selector, Specification, Specification Table, Table |
| **Interactive & utility** | CTA Buttons, Country Selector, Dealer Locator, Woosmap Dealers, Fragment, Form |
| **Site-wide (not authored per page)** | Header, Footer, Error Page |
...
4. **Add body blocks** (Cards, Text with Image, Specification, etc.) in the order you want them to appear on the page. Check each block's entry in the [Blocks Library](../pages/blocks.html) if you're not sure which one fits — the "when to use it" note is there to help you pick.
...
These show the **table shape** you type directly into your da.live document. The left column of the first row is always the block's name. Everything below it is the block's content, one row per repeatable item (e.g. one row per card). The [Blocks Library](../pages/blocks.html) has one of these for every block — the ones below are the most commonly used.
...
**Which block should I use — Cards, Teaser, or News/Stories?**
They all use the same image + text row pattern, but style the heading differently and imply different content: Cards for a general feature/model grid, Teaser for smaller secondary callouts, News/Stories for a news feed. See each one's "when to use it" note in the [Blocks Library](../pages/blocks.html).
